...
フォームの入力文字数表示機能はHTMLの textarea 要素または type 属性が text の input 要素にのみ対応しています。これらはテキスト入力ボックスを構成する最も一般的なHTML要素ですが、ウェブページによっては別の要素が使われていることがあり、その場合は動作しません。
