...
入り組んだHTML構成・JavaScript制御の影響やセキュリティーの制約などにより、ウェブページによっては当プラグインがうまく作動しないことがあります。

また、結果は、開いているウェブページのHTMLに挿入する仕組みで表示しています。そのため、当該ページのスタイル指定などの影響を受けて変形したり、一部や全部が表示されなかったりする場合があります。

なお、ネットワーク上ではなくお使いのPC上のファイル（「file[]()://」プロトコルで示されるローカルファイル）を対象に作動させるには、ブラウザーの ``拡張機能 → 〈字数〉詳細`` 画面 で「ファイルのURLへのアクセスを許可する」を有効にしてください。
