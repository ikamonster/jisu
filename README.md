...
なお、フォーム入力の自動計数機能は textarea 要素、または type 属性が text, search, tel, url, email, number, file の input 要素にのみ対応しています。ウェブページによっては、テキスト入力フォームがこれらの要素でない場合があります（リッチテキスト対応フォーム等）。また、セキュリティーの制約などにより動作しないこともあります。
