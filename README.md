...
- 自動計数モード時、フォームに入力した文字数も自動的に数えることができます（要設定）
...
テキストの選択操作中、またはフォームへのテキスト入力中（要設定）に、文字数を自動的に数えて即時表示する機能です。
...
#### フォームに入力した文字数が表示されません

オプション画面の「フォーム入力も自動計数」が「する」になっていること、自動計数モードに移行していることをご確認ください。

なお、フォーム入力の自動計数機能は textarea 要素、または type 属性が text, search, tel, url, email, number, file のいずれかの input 要素にのみ対応しています。ウェブページによっては、テキスト入力フォームがこれらの要素でない場合があります（リッチテキスト対応フォーム等）。また、セキュリティーの制約などにより動作しないこともあります。
...
- （開発中／公開申請中）
  - フォーム入力テキストの自動計数機能を追加
  - 入れ子のフレーム内の文字を数えられるよう修正
- バージョン 2.3
