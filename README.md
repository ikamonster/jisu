...
オプション画面の「フォーム入力も自動計数」が「する」になっていることと、自動計数モードに移行していることをご確認ください。

なお、フォーム入力の自動計数機能は textarea 要素、または type 属性が text の input 要素にのみ対応しています。ウェブページによっては、テキスト入力領域がこれらの要素でない場合があります（リッチテキスト対応フォーム等）。また、セキュリティーの制約などにより動作しないこともあります。
