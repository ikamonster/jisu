...
文字カウントにおいては、対象のひとつである日本語の書き方はとりわけ多様であり、約物の使用規則も統一されておらず、さらに外国語を交ぜて書くことすら可能です。また、文学的・詩的表現においては文とそうでないものとの境界が曖昧です。
...
- 古典的測定方法そのものの限界
	...
	- そもそもいくつもの測定方法が考案されていること自体、どの方法も完全でないことを示しています。テキストの読みやすさが単語や音節の数といった量的要因だけで決まるわけでないことは経験則からも明らかでしょう。結局、読みやすさの最終的な判断は人間が下すしかありません。とはいえ少なくとも、測定値はその補助として役立つ可能性があります。
...
- OSに音声読み上げ機能が備わっており、指定言語の音声がインストールされている必要があります。PCの音量設定やスピーカー／ヘッドホンの接続もご確認ください。
- 音声データのロードなど音声エンジン側の処理の都合により、読み上げが遅れたりキャンセルされたりすることがあります（特に最初の実行時）。
- クラウドの音声を選んでいる場合、オフラインでは発声できません。
- ウェブページや他の拡張機能が音声読み上げをしている場合には、発声処理が重なってうまく鳴らなかったり聴こえにくくなったりすることがあります。

## オプション設定が反映されません

- 当拡張機能を再度実行する、ページをリロードする、ブラウザーを再起動する、のいずれかをお試しください。
- それでも反映されない場合は、設定情報が壊れている可能性があります。お手数ですが、当拡張機能をいったん削除して再度インストールしてください。
- 音声読み上げの音量と音声はブラウザー間で同期されません。適切な値や可能な選択肢が環境によって異なるためです。
...
## 自分の使っているブラウザーに対応していません

開発に割ける時間・資源・費用が限られていることから、当拡張機能は原則として次の条件を満たすウェブ拡張機能プラットフォームのみを対象としています。

1. 普及率の高いPC用モダンブラウザー最新版
2. Manifest V3と最新のAPIに対応しておりコード互換性が高い
3. 特殊な開発環境を要さず、ほぼ無料で配布できる

Firefox版は、FirefoxがManifest V3に正式対応し次第リリースする予定です。ただし、利用できるAPIの違いから、一部の機能を省くかもしれません（2022年6月現在）。  
Safariは３を満たさないため、今のところ対応する予定はありません。

<br>

# 改版履歴
- バージョン 3.2（公開申請中）
  - 「テキストを数える」（単語か文字か、検出された言語に適したほうを数える）機能を追加
  - 認識文字種に点字を追加
- バージョン 3.1
  - 文字を数える
    - 句点とみなす文字の設定を追加
