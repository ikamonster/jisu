...
OSに音声読み上げ機能が備わっており、指定言語の音声がインストールされている必要があります。音量設定やスピーカーの接続もご確認ください。

音声データのロードなど、OS側の処理の都合により読み上げが遅れたり実行されなかったりする場合もあります。

## オプション設定が反映されません

当拡張機能を再度実行する、ページをリロードする、ブラウザーを再起動する、のいずれかをお試しください。それでも反映されない場合は、設定情報が壊れている可能性があります。お手数ですが、当拡張機能をいったん削除して再度インストールしてください。
...
開発に割ける時間・資源・費用が限られていることから、当拡張機能は原則として次の条件を満たすブラウザーのみを対象としています。

1. 普及率の高いPC用モダンブラウザー最新版
2. Manifest V3と最新のAPIに対応しておりコード互換性が高い
3. 特殊な開発環境を要さず、無料で配布できる

Firefox版は、FirefoxがManifest V3に対応し次第リリースする予定です。ただし、利用できるAPIの違いから、一部の機能が省かれる可能性があります（2022年6月現在）。  
