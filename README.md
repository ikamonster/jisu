...
## 「テキストを数える」を実行すると、和文のページなのに単語を数えたり、欧文のページなのに文字を数えたりしてしまいます

「テキストを数える」において文字を数えるのは、原則として単語単位での分かち書きをしない、チベット語・日本語・クメール語・ラーオ語・ビルマ語・パーリ語・サンスクリット・タイ語・中国語が対象です（バージョン3.2現在）。これら以外の言語では単語を数えます。

言語はブラウザーが検出します。ブラウザーは独自の言語検出アルゴリズムを備えていますが、ウェブページに使用言語が明記されている場合、それを最も重視します。見かけの言語と検出された言語が異なる場合、その記述が不適切であることがほとんどです（ウェブサイトを制作・配信している国の公用語が一律設定されているなど）。また、複数の言語が混在するページでは、必然的に誤検出しやすくなります。
