...
また、選択範囲が「親と子のフレームをまたぐ」あるいは「フレーム内の本文と入力フォームとをまたぐ」場合には、APIが完全な文字列を返してくれず、これもやむを得ず不正確な結果となることがあります。今日ではフレームは動画・地図・SNSタグ・広告などの第三者リソースの埋め込みに使われることが専らで、問題となることは少ないはずですが、仕様としてあらかじめご了承ください。
