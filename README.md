...
Google Docs は、テキスト編集領域をCanvas要素とし、文字列やUI操作を独自に処理・描画している高度なウェブアプリケーションです。当拡張機能が対象としている、DOM上のテキストノードおよびその選択処理とはまるで性質が異なります（2022年現在）。

他の同様のウェブアプリケーションや、より小規模であってもJavaScriptで制御された独自のテキスト領域（SNSの投稿画面や、Wikiなどのリッチテキスト編集画面に多い）に対しては、当拡張機能が正常に動作しないことがあります。
